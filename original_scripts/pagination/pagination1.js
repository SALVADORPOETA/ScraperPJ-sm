...
async function scrapeCandidatos() {
  ...
  // Expandir el panel "Ministras y ministros de la Suprema Corte de Justicia de la Nación"
  ...
  while (pageNumber <= 7) {
    ...
    const urls = await page.evaluate(() => {
      const enlaces1 = Array.from(document.querySelectorAll('.div-FConsulta'))
      return enlaces1
        .map((enlace) => {
    ...
    // if (pageNumber % 25 === 0 || pageNumber === 7) {
    //   const partFileName = path.join(
    //     __dirname,
    //     `enlaces1_parte_${pageNumber}.json`
    //   )
  ...
  const finalPath = path.join(__dirname, 'enlaces1.json')
