...
async function scrapeCandidatos() {
  ...
  // Expandir el panel "Magistradas y magistrados de la Sala Superior del Tribunal Electoral del Poder Judicial de la Federación"
  ...
  while (pageNumber <= 2) {
    ...
    const urls = await page.evaluate(() => {
      const enlaces3 = Array.from(document.querySelectorAll('.div-FConsulta'))
      return enlaces3
        .map((enlace) => {
    ...
    if (pageNumber % 25 === 0 || pageNumber === 2) {
      const partFileName = path.join(
        __dirname,
        `enlaces3_parte_${pageNumber}.json`
      )
  ...
  const finalPath = path.join(__dirname, 'enlaces3.json')
