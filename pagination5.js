...
async function scrapeCandidatos() {
  ...
  // Expandir el panel "Magistradas y magistrados de Circuito"
  ...
  for (let header of headers) {
    ...
    if (text.includes('Magistradas y magistrados de Circuito')) {
      await header.click()
      break
    }
  }
  ...
  while (pageNumber <= 164) {
    console.log(`📄 Cargando página ${pageNumber}...`)

    const urls = await page.evaluate(() => {
      const enlaces5 = Array.from(document.querySelectorAll('.div-FConsulta'))
      return enlaces5
        .map((enlace) => {
    ...
    if (pageNumber % 25 === 0 || pageNumber === 164) {
      const partFileName = path.join(
        __dirname,
        `enlaces5_parte_${pageNumber}.json`
      )
  ...
  const finalPath = path.join(__dirname, 'enlaces5.json')
