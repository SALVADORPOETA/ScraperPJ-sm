...
async function scrapeCandidatos() {
  ...
  // Expandir el panel "Juezas y jueces de Distrito"
  ...
  while (pageNumber <= 157) {
    console.log(`📄 Cargando página ${pageNumber}...`)

    const urls = await page.evaluate(() => {
      const enlaces6 = Array.from(document.querySelectorAll('.div-FConsulta'))
      return enlaces6
        .map((enlace) => {
  ...
  const finalPath = path.join(__dirname, 'enlaces6.json')
  fs.writeFileSync(finalPath, JSON.stringify(totalLinks, null, 2))
  console.log(
    `🎉 ¡Listo! Se guardaron ${totalLinks.length} enlaces en ${finalPath}`
  )
